...
$exe = Join-Path $PSScriptRoot "GamingDaily.exe"
try {
  $action  = New-ScheduledTaskAction -Execute $exe
  $trigger = New-ScheduledTaskTrigger -Daily -At 8:00am
  $set     = New-ScheduledTaskSettingsSet -StartWhenAvailable -ExecutionTimeLimit (New-TimeSpan -Minutes 10)
  Register-ScheduledTask -TaskName "Gaming Daily" -Action $action -Trigger $trigger -Settings $set -Description "Ouvre le magazine jeu video chaque matin" -Force | Out-Null
